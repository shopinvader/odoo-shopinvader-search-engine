...
class TestProductProduct(StockCommonCase):
    ...
    def test_config_only_qty(self):
        self.index.stock_level_config = "only_qty"
        self.assertEqual(self.product.stock_data, {"global": {"qty": 0.00}})

    def test_config_only_state(self):
        self.index.stock_level_config = "only_state"
        self._add_stock_to_product(self.product, self.loc_1, 5)
        self.assertEqual(
            self.product.stock_data,
    ...
    def test_config_state_and_qty(self):
        self.index.stock_level_config = "state_and_qty"
        self.assertEqual(
            self.product.stock_data,
